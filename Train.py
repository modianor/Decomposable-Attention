...
def train():
    ...
    for epoch in range(arg.num_epochs):
        ...
        for batch in batches:
            ...
            _, batch_loss, batch_acc, batch_e = sess.run([model.train, model.loss, model.acc, model.e],
                                                         feed_dict=feed_dict)
            total_loss += batch_loss * batch_nums
            total_acc += batch_acc * batch_nums

            if not isinstance(total_e, ndarray):
                total_e = batch_e
            else:
                total_e = np.vstack((total_e, batch_e))
            print(batch_e.shape)
            print('batch_e : {}'.format(batch_e))
            # evaluta on devset
        ...
        np.save('data/e-epoch{}.npy'.format(epoch + 1), total_e)
...
if __name__ == '__main__':
    ...
    model = Decomposable(arg.seq_length, arg.n_vocab, arg.embedding_size, arg.hidden_size, arg.attention_size,
                         arg.n_classes, \
